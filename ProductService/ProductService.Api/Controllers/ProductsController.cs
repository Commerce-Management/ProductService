...
public class ProductsController(IProductService productService) : ControllerBase
{
    ...
    [HttpGet("paginated")]
    public async Task<ActionResult> GetProductsPaginated([FromQuery] int page = 1, [FromQuery] int limit = 10)
    {
        try
        {
            var (items, totalCount) = await productService.GetProductsPaginatedAsync(page, limit);
        ...
        {
            Log.Error(ex, "Ошибка при получении продуктов (paginated)");
            return StatusCode(500, new
            {
                Error = ex.Message,
                Exception = ex.GetType().Name,
        ...
        [FromQuery] int limit = 10)
    {
        ...
        try
        {
            var products = await productService.SearchProduct(query, page, limit);
            if (!products.Any())
                return NotFound(new
                {
    ...
    public async Task<ActionResult<IEnumerable<GetProductDto>>> GetProductsByCategories(
        ...
        [FromQuery] int limit = 10)
    {
        if (categoryIds == null || categoryIds.Length == 0)
            return BadRequest(new
            {
                Error = "categoryIds cannot be empty.",
                Exception = "ArgumentException",
                StackTrace = string.Empty
            });
        ...
        try
        {
            var products = await productService.GetProductsByCategoriesAsync(categoryIds, page, limit);
            if (!products.Any())
                return NotFound(new
                {
                    Error = "Products not found for given categories.",
                    Exception = "NotFoundException",
                    StackTrace = string.Empty
                });
        ...
        {
            Log.Error(ex, "Ошибка при получении продуктов по категориям");
            return StatusCode(500, new
            {
                Error = ex.Message,
                Exception = ex.GetType().Name,
    ...
    public async Task<ActionResult<IEnumerable<GetProductDto>>> GetAllShopProducts([FromQuery] Guid shopId, [FromQuery] int page = 1, [FromQuery] int limit = 10)
    {
        try
        {
            var result = await productService.GetAllShopProductsAsync(shopId, page, limit);
            if (result.Any()) return Ok(result);
    ...
    public async Task<IActionResult> DeleteProduct([FromQuery] Guid shopId, Guid productId)
    {
        var userIdClaim = User.FindFirst(ClaimTypes.NameIdentifier)?.Value;
        if (string.IsNullOrEmpty(userIdClaim) || !Guid.TryParse(userIdClaim, out var userId))
            return Unauthorized(new
